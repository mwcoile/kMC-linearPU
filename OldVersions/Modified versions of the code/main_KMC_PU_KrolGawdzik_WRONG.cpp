...
int main() {
    ...
    // KMC step
    double kf= 1.5E-3; // A_kf*exp(-Ea_kf/(R*T)); // adding a to b
    ...
    double simulation_time = 60*120; // [=] seconds
    ...
    while (time<simulation_time) {
        ...
        // update total rate
        double total_rate =c*(moleculesA+chainsA)*(moleculesB+chainsB)/Na; // molecules/s 2*molecules because each monomer is bifunctional
        ...
        double whichA = r3*(chainsA+moleculesA);
        double r4 = 1.0*std::rand()/RAND_MAX;
        double whichB = r4*(chainsB+moleculesB); 
        front=false;
        back=false;
        // case for which two monomers react to form new chain
        if (whichA<moleculesA && whichB<moleculesB) {
            chain newchain;
        ...
        // case for which one A monomer reacts with one B chain
        else if (whichA<moleculesA && whichB>moleculesB) {
            // STEP 1: Select B chain
            int Bselect = (int) (whichB-moleculesB);
            ...
            if (all_chains[selected_chain].v.size()%2==1 && r2>0.5) {
                // do nothing whatsoever
            }
            else if (1 == all_chains[selected_chain].v.back() && back) {
        ...
        // case for which 1 B monomer reacts with 1 A chain
            ...
            int Aselect = (int) (whichA-moleculesA);
            int selected_chain = 0;
            int countA = 0;
            while (selected_chain<all_chains.size()) {
            ...
            else if (0 == all_chains[selected_chain].v.back() && back) {
